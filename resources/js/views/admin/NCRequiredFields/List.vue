<template>
    <div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div v-if="requiredFieldsConfigs.length && !isLoading">
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td>
                                        {{
                                            call_sub_category
                                                ? call_sub_category.call_sub_category_name
                                                : ""
                                        }}
                                    </td>
                                    <td>
...
export default {
    ...
    methods: {
        async fetchRequiredFieldsConfigs() {
            this.isLoading = true;
            try {
                const response = await axios.get("/required-fields-configs");
                this.requiredFieldsConfigs = response.data;
            } catch (error) {
        ...
        async deleteRequiredFieldsConfig(id) {
            try {
                if (
                    confirm(
                        "Are you sure you want to delete this call sub-category?"
                    )
                ) {
                    await axios.delete(`/required-fields-configs/${id}`);
                    await this.fetchRequiredFieldsConfigs();
                }
            } catch (error) {
    ...
    watch: {
        requiredFieldsConfigs(newValue) {
            if (newValue.length) {
