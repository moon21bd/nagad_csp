    ...
    <div>
        <form>
            <div class="form-group">
                ...
                <select
                    v-model="requiredFields.call_type_id"
            ...
            <div class="form-group">
                ...
                <select
                    v-model="requiredFields.call_category_id"
            ...
            <div class="form-group">
                ...
                <select
                    v-model="requiredFields.call_sub_category_id"
            ...
            <ValidationObserver v-slot="{ invalid }">
                <div v-if="selectedField">
                    <label :for="selectedField.input_field_name">{{
                        selectedField.input_field_name
                    }}</label>

                    <ValidationProvider
                        :rules="
                            generateValidationRules(
                                selectedField.input_validation
                            )
                        "
                        v-slot="{ errors }"
                    >
                        <component
                            ...
                            :field="selectedField"
                        ></component>
                        <span v-show="errors.length" class="text-danger">{{
                            errors[0]
                        }}</span>
                    </ValidationProvider>
                </div>
...
Validator.extend("max", {
    getMessage: (field, [length]) =>
        `${field} may not be greater than ${length} characters.`,
...
export default {
    ...
    computed: {
        selectedField() {
            console.log(
                "selectedField",
                this.requiredFieldsInfo.find(
                    (field) =>
                        field.call_sub_category_id ===
                        this.selectedSubCategoryId
                )
            );
            return this.requiredFieldsInfo.find(
                (field) =>
                    field.call_sub_category_id === this.selectedSubCategoryId
            );
        },
        inputComponent() {
    ...
    methods: {
        ...
        async fetchCategories() {
            try {
                const response = await axios.get(
                    `/get-category/${this.requiredFields.call_type_id}`
                );
        ...
        async fetchSubCategory() {
            try {
                const response = await axios.get(
                    `/call-sub-by-call-cat-id/${this.requiredFields.call_type_id}/${this.requiredFields.call_category_id}`
                );
        ...
        async fetchRequiredFields() {
            try {
                const response = await axios.get(
                    `/get-required-fields/${this.requiredFields.call_type_id}/${this.requiredFields.call_category_id}/${this.requiredFields.call_sub_category_id}`
                );
                this.requiredFieldsInfo = response.data;
                this.selectedSubCategoryId =
                    this.requiredFields.call_sub_category_id;
            } catch (error) {
