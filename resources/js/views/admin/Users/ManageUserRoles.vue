<template>
    <div>
        <div class="common-heading d-flex align-items-center mb-3">
            ...
            <h1 class="title m-0">Manage User Roles</h1>
        </div>
...
export default {
    ...
    methods: {
        ...
        fetchUserRoles() {
            this.isLoading = true;
            axios
                .get(`/user/${this.id}/roles`)
                .then((response) => {
                    this.userRoles = response.data;
                    this.selectedRoles = this.userRoles.map(
                        (role) => role.name
                    );
                })
                .finally(() => {
        ...
        removeRole(roleId) {
            this.isLoading = true;
            axios
                .delete(`/user/${this.id}/roles/${roleId}`)
                .then(() => {
                    this.fetchUserRoles();
                })
                .catch((error) => {
