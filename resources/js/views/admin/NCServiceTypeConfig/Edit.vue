...
export default {
    ...
    mounted() {
        ...
        if (this.$route.params.id) {
            this.fetchFormData(this.$route.params.id);
        }
    },
    methods: {
        checkNegative(group) {
            ...
            try {
                ...
                if (this.$route.params.id) {
                    ...
                    response = await axios.put(
                        `/service-type-config/${this.$route.params.id}`,
                        payload
                    );
                } else {
                    // Create mode: Send a POST request
        ...
        async fetchSmsConfigs() {
            try {
                const response = await axios.get("/get-sms-configs");
        ...
        async fetchEmailConfigs() {
            try {
                const response = await axios.get("/get-email-configs");
        ...
        async fetchFormData(id) {
            try {
                const response = await axios.get(`/service-type-config/${id}`);
                ...
                this.configurationInfos = {
                    ...this.configurationInfos,
                    ...data,
                    selectedGroups: data.selectedGroups.map((group) => ({
                        ...group,
                        isChecked: true,
                    })),
                    selectedNotificationChannels:
                        data.selectedNotificationChannels,
                };
                ...
                this.groups = this.groups.map((group) => ({
                    ...group,
                    isChecked: data.selectedGroups.some(
                        (selectedGroup) => selectedGroup.id === group.id
                    ),
                }));

                // Ensure notification channels are set correctly
        ...
        addPopupMessage() {
            if (
                ...
                this.$showToast(
                    `Maximum of ${this.maxMessages} messages allowed.`,
                    { type: "warning" }
                );
            }
        },
